...
/*******************************************************************************/
ReturnCode RfalNfcClass::rfalIsoDepReSendControlMsg(void)
{
...
/*******************************************************************************/
void RfalNfcClass::rfalIsoDepInitializeWithParams(rfalComplianceMode compMode, uint8_t maxRetriesR, uint8_t maxRetriesSnWTX, uint8_t maxRetriesSWTX, uint8_t maxRetriesSDSL, uint8_t maxRetriesI, uint8_t maxRetriesRATS)
{
...
ReturnCode RfalNfcClass::rfalIsoDepDeselect(void)
{
  ...
  EXIT_ON_ERR(ret, rfalIsoDepStartDeselect());
...
/*******************************************************************************/
ReturnCode RfalNfcClass::rfalIsoDepStartDeselect(void)
{
...
/*******************************************************************************/
ReturnCode RfalNfcClass::rfalIsoDepGetDeselectStatus(void)
{
  ReturnCode ret;
  bool dummyB;

  EXIT_ON_BUSY(ret, isoDepDataExchangePCD(gIsoDep.rxLen, &dummyB));
...
/*******************************************************************************/
bool RfalNfcClass::rfalIsoDepIsRats(const uint8_t *buf, uint8_t bufLen)
{
  if (buf != NULL) {
...
/*******************************************************************************/
bool RfalNfcClass::rfalIsoDepIsAttrib(const uint8_t *buf, uint8_t bufLen)
{
  if (buf != NULL) {
...
/*******************************************************************************/
ReturnCode RfalNfcClass::rfalIsoDepListenStartActivation(rfalIsoDepAtsParam *atsParam, const rfalIsoDepAttribResParam *attribResParam, const uint8_t *buf, uint16_t bufLen, rfalIsoDepListenActvParam actParam)
{
  uint8_t *txBuf;
  uint8_t bufIt;
  ...
  return rfalRfDev->rfalTransceiveBlockingTx(txBuf, bufIt, (uint8_t *)actParam.rxBuf, sizeof(rfalIsoDepBufFormat), actParam.rxLen, RFAL_TXRX_FLAGS_DEFAULT, RFAL_FWT_NONE);
}

/*******************************************************************************/
ReturnCode RfalNfcClass::rfalIsoDepListenGetActivationStatus(void)
{
  ReturnCode err;
  uint8_t *txBuf;
  uint8_t bufIt;
  ...
  err = rfalRfDev->rfalGetTransceiveStatus();
  switch (err) {
    /*******************************************************************************/
    case ERR_NONE:
      break;
    ...
    case ERR_LINK_LOSS:
    case ERR_BUSY:
    case ERR_SLEEP_REQ: /* S(DSL) handled by lower layer should cause an error reported */
      return err;
  ...
  if (gIsoDep.state == ISODEP_ST_PICC_ACT_ATS) {
    ...
    if ((((uint8_t *)gIsoDep.actvParam.rxBuf)[RFAL_ISODEP_PPS_STARTBYTE_POS] & RFAL_ISODEP_PPS_MASK) == RFAL_ISODEP_PPS_SB) {
      ...
      rfalRfDev->rfalTransceiveBlockingTx(txBuf, bufIt, (uint8_t *)gIsoDep.actvParam.rxBuf, sizeof(rfalIsoDepBufFormat), gIsoDep.actvParam.rxLen, RFAL_TXRX_FLAGS_DEFAULT, RFAL_FWT_NONE);

      /*******************************************************************************/
      /* Exchange the bit rates if requested */
      if (dri != RFAL_BR_KEEP) {
        rfalRfDev->rfalSetBitRate(dsi, dri);
...
#if RFAL_FEATURE_ISO_DEP_LISTEN
/*******************************************************************************/
ReturnCode RfalNfcClass::rfalIsoDepDataExchangePICC(void)
{
  uint8_t rxPCB;
  ReturnCode ret;
  ...
  switch (gIsoDep.state) {
    ...
    case ISODEP_ST_PICC_SDSL:
      ...
      if (!(rfalRfDev->rfalIsTransceiveInTx())) { /* Wait until DSL response has been sent */
...
/*******************************************************************************/
ReturnCode RfalNfcClass::rfalIsoDepStartRATS(rfalIsoDepFSxI FSDI, uint8_t DID, rfalIsoDepAts *ats, uint8_t *atsLen)
{
  rfalTransceiveContext ctx;
...
/*******************************************************************************/
ReturnCode RfalNfcClass::rfalIsoDepGetRATSStatus(void)
{
  ReturnCode ret;
...
/*******************************************************************************/
ReturnCode RfalNfcClass::rfalIsoDepStartPPS(uint8_t DID, rfalBitRate DSI, rfalBitRate DRI, rfalIsoDepPpsRes *ppsRes)
{
  rfalTransceiveContext ctx;
...
/*******************************************************************************/
ReturnCode RfalNfcClass::rfalIsoDepGetPPSSTatus(void)
{
  ReturnCode ret;
...
ReturnCode RfalNfcClass::rfalIsoDepStartATTRIB(const uint8_t *nfcid0, uint8_t PARAM1, rfalBitRate DSI, rfalBitRate DRI, rfalIsoDepFSxI FSDI, uint8_t PARAM3, uint8_t DID, const uint8_t *HLInfo, uint8_t HLInfoLen, uint32_t fwt, rfalIsoDepAttribRes *attribRes, uint8_t *attribResLen)
{
  rfalTransceiveContext ctx;
...
/*******************************************************************************/
ReturnCode RfalNfcClass::rfalIsoDepGetATTRIBStatus(void)
{
  ReturnCode ret;
...
/*******************************************************************************/
ReturnCode RfalNfcClass::rfalIsoDepPollAStartActivation(rfalIsoDepFSxI FSDI, uint8_t DID, rfalBitRate maxBR, rfalIsoDepDevice *isoDepDev)
{
  ReturnCode ret;
  ...
  rfalRfDev->rfalSetErrorHandling(ERRORHANDLING_EMD);
...
/*******************************************************************************/
ReturnCode RfalNfcClass::rfalIsoDepPollAGetActivationStatus(void)
{
  ReturnCode ret;
  uint8_t msgIt;
  rfalBitRate maxBR;
    ...
    case ISODEP_ST_PCD_ACT_RATS:
      ...
      if (ret != ERR_BUSY) {
        if (ret != ERR_NONE) {
          ...
          rfalRfDev->rfalSetGT(rfalConvMsTo1fc(gIsoDep.actvDev->info.SFGT));
          rfalRfDev->rfalFieldOnAndStartGT();
    ...
    case ISODEP_ST_PCD_ACT_PPS:
      ...
      if (ret != ERR_BUSY) {
        ...
        if (ret == ERR_NONE) {
          ...
          rfalRfDev->rfalSetBitRate(gIsoDep.actvDev->info.DRI, gIsoDep.actvDev->info.DSI);
        } else {
...
/*******************************************************************************/
ReturnCode RfalNfcClass::rfalIsoDepPollBStartActivation(rfalIsoDepFSxI FSDI, uint8_t DID, rfalBitRate maxBR, uint8_t PARAM1, const rfalNfcbListenDevice *nfcbDev, const uint8_t *HLInfo, uint8_t HLInfoLen, rfalIsoDepDevice *isoDepDev)
{
  ReturnCode ret;
  uint32_t tr2;
  ...
  if (gIsoDep.compMode == RFAL_COMPLIANCE_MODE_EMV) {
    ...
    if ((rfalRfDev->rfalGetFDTPoll()) < tr2) {
...
/*******************************************************************************/
ReturnCode RfalNfcClass::rfalIsoDepPollBGetActivationStatus(void)
{
  ReturnCode ret;
  uint8_t mbli;
...
ReturnCode RfalNfcClass::rfalIsoDepPollHandleSParameters(rfalIsoDepDevice *isoDepDev, rfalBitRate maxTxBR, rfalBitRate maxRxBR)
{
  ...
  EXIT_ON_ERR(ret, rfalRfDev->rfalTransceiveBlockingTxRx((uint8_t *)&sParam, (RFAL_ISODEP_SPARAM_HDR_LEN + (uint16_t)it), (uint8_t *)&sParam, sizeof(rfalIsoDepControlMsgSParam), &rcvLen, RFAL_TXRX_FLAGS_DEFAULT, ISODEP_FWT_DEACTIVATION));
