...
#define nfcDepReEnableRx( rxB, rxBL, rxL )       rfalRfDev->rfalTransceiveBlockingTx( NULL, 0, (rxB), (rxBL), (rxL), ( RFAL_TXRX_FLAGS_DEFAULT | (uint32_t)RFAL_TXRX_FLAGS_NFCIP1_ON ), RFAL_FWT_NONE )
...
/*******************************************************************************/
bool RfalNfcClass::nfcipDxIsSupported(uint8_t Dx, uint8_t BRx, uint8_t BSx)
{
  uint8_t Bx;
...
/*******************************************************************************/
ReturnCode RfalNfcClass::nfcipTxRx(rfalNfcDepCmd cmd, uint8_t *txBuf, uint32_t fwt, uint8_t *paylBuf, uint8_t paylBufLen, uint8_t *rxBuf, uint16_t rxBufLen, uint16_t *rxActLen)
{
  ReturnCode ret;
...
/*******************************************************************************/
ReturnCode RfalNfcClass::nfcipDEPControlMsg(uint8_t pfb, uint8_t RTOX)
{
  uint8_t        ctrlMsg[20];
  uint32_t       fwt;
...
/*******************************************************************************/
void RfalNfcClass::nfcipClearCounters(void)
{
...
/*******************************************************************************/
ReturnCode RfalNfcClass::nfcipInitiatorHandleDEP(ReturnCode rxRes, uint16_t rxLen, uint16_t *outActRxLen, bool *outIsChaining)
{
  ReturnCode ret;
  uint8_t    nfcDepLen;
  uint8_t    rxMsgIt;
...
/*******************************************************************************/
ReturnCode RfalNfcClass::nfcipTargetHandleRX(ReturnCode rxRes, uint16_t *outActRxLen, bool *outIsChaining)
{
  ReturnCode ret;
  uint8_t    nfcDepLen;
  uint8_t    rxMsgIt;
...
/*******************************************************************************/
ReturnCode RfalNfcClass::nfcipTx(rfalNfcDepCmd cmd, uint8_t *txBuf, uint8_t *paylBuf, uint16_t paylLen, uint8_t pfbData, uint32_t fwt)
{
  uint16_t txBufIt;
  uint8_t *txBlock;
  uint8_t *payloadBuf;
 ...
 ******************************************************************************
 */
void RfalNfcClass::nfcipConfig(const rfalNfcDepConfigs *cfg)
{
  if (cfg == NULL) {
    return;
  }
 ...
 ******************************************************************************
 */
ReturnCode RfalNfcClass::nfcipRun(uint16_t *outActRxLen, bool *outIsChaining)
{
  ReturnCode ret;
...
/*******************************************************************************/
void RfalNfcClass::rfalNfcDepSetDeactivatingCallback(rfalNfcDepDeactCallback pFunc)
{
...
/*******************************************************************************/
void RfalNfcClass::rfalNfcDepInitialize(void)
{
  nfcipLogD(" NFCIP Ini() \r\n");
 ...
 ******************************************************************************
 */
void RfalNfcClass::nfcipSetDEPParams(rfalNfcDepDEPParams *DEPParams)
{
...
/*******************************************************************************/
bool RfalNfcClass::rfalNfcDepTargetRcvdATR(void)
{
...
/*******************************************************************************/
bool RfalNfcClass::rfalNfcDepIsAtrReq(const uint8_t *buf, uint16_t bufLen, uint8_t *nfcid3)
{
  uint8_t msgIt;
...
/*******************************************************************************/
ReturnCode RfalNfcClass::nfcipTargetHandleActivation(rfalNfcDepDevice *nfcDepDev, uint8_t *outBRS)
{
  ReturnCode ret;
  uint8_t    msgIt;
  ...
  *outBRS = RFAL_NFCDEP_BRS_MAINTAIN;                   /* set out BRS to be maintained */
  ...
  if (gNfcip.rxBuf[msgIt] == (uint8_t)NFCIP_CMD_PSL_REQ) {
    ...
    *outBRS = gNfcip.rxBuf[msgIt++];                  /* assign output BRS value      */
    ...
    rfalRfDev->rfalSetBitRate(RFAL_BR_KEEP, gNfcip.nfcDepDev->info.DSI);
...
/*******************************************************************************/
ReturnCode RfalNfcClass::rfalNfcDepATR(const rfalNfcDepAtrParam *param, rfalNfcDepAtrRes *atrRes, uint8_t *atrResLen)
{
  ReturnCode        ret;
...
/*******************************************************************************/
ReturnCode RfalNfcClass::rfalNfcDepPSL(uint8_t BRS, uint8_t FSL)
{
  ReturnCode ret;
  uint16_t   rxLen;
  uint8_t    msgIt;
...
/*******************************************************************************/
ReturnCode RfalNfcClass::rfalNfcDepDSL(void)
{
  ReturnCode ret;
...
/*******************************************************************************/
ReturnCode RfalNfcClass::rfalNfcDepRLS(void)
{
  ReturnCode ret;
...
/*******************************************************************************/
ReturnCode RfalNfcClass::rfalNfcDepInitiatorHandleActivation(rfalNfcDepAtrParam *param, rfalBitRate desiredBR, rfalNfcDepDevice *nfcDepDev)
{
  ReturnCode ret;
  uint8_t    maxRetyrs;
  uint8_t    PSL_BRS;
  ...
  rfalRfDev->rfalGetBitRate(&nfcDepDev->info.DSI, &nfcDepDev->info.DRI);
  ...
  * */
  ...
  if (sendPSL) {
    ...
    if (nfcDepDev->info.DSI != desiredBR) {
      ...
      rfalRfDev->rfalSetBitRate(nfcDepDev->info.DSI, nfcDepDev->info.DRI);
    }
...
/*******************************************************************************/
uint32_t RfalNfcClass::rfalNfcDepCalculateRWT(uint8_t wt)
{
...
{
  return rfalRfDev->rfalTransceiveBlockingTx(txBuf, txBufLen, gNfcip.rxBuf, gNfcip.rxBufLen, gNfcip.rxRcvdLen, (RFAL_TXRX_FLAGS_DEFAULT | (uint32_t)RFAL_TXRX_FLAGS_NFCIP1_ON), ((fwt == NFCIP_NO_FWT) ? RFAL_FWT_NONE : fwt));
}
...
ReturnCode RfalNfcClass::nfcipDataRx(bool blocking)
{
  ...
  if (blocking) {
    ret = rfalRfDev->rfalTransceiveBlockingRx();
  } else {
...
/*******************************************************************************/
ReturnCode RfalNfcClass::rfalNfcDepListenStartActivation(const rfalNfcDepTargetParam *param, const uint8_t *atrReq, uint16_t atrReqLength, rfalNfcDepListenActvParam rxParam)
{
  ReturnCode        ret;
  rfalNfcDepConfigs cfg;
  ...
  rfalRfDev->rfalGetBitRate(&rxParam.nfcDepDev->info.DSI, &rxParam.nfcDepDev->info.DRI);
...
/*******************************************************************************/
ReturnCode RfalNfcClass::rfalNfcDepListenGetActivationStatus(void)
{
  ReturnCode err;
  uint8_t    BRS;
    ...
    case ERR_NONE:
      ...
      if (BRS != RFAL_NFCDEP_BRS_MAINTAIN) {
        ...
        if (gNfcip.cfg.commMode == RFAL_NFCDEP_COMM_ACTIVE) {
          EXIT_ON_ERR(err, rfalRfDev->rfalSetMode(RFAL_MODE_LISTEN_ACTIVE_P2P, gNfcip.nfcDepDev->info.DRI, gNfcip.nfcDepDev->info.DSI));
        } else {
          EXIT_ON_ERR(err, rfalRfDev->rfalSetMode(((RFAL_BR_106 == gNfcip.nfcDepDev->info.DRI) ? RFAL_MODE_LISTEN_NFCA : RFAL_MODE_LISTEN_NFCF), gNfcip.nfcDepDev->info.DRI, gNfcip.nfcDepDev->info.DSI));
        }
      }
      break;
...
/*******************************************************************************/
ReturnCode RfalNfcClass::rfalNfcDepStartTransceive(const rfalNfcDepTxRxParam *param)
{
...
/*******************************************************************************/
ReturnCode RfalNfcClass::rfalNfcDepGetTransceiveStatus(void)
{
  return nfcipRun(gNfcip.rxRcvdLen, gNfcip.isChaining);
}

/*******************************************************************************/
void RfalNfcClass::rfalNfcDepPdu2BLockParam(rfalNfcDepPduTxRxParam pduParam, rfalNfcDepTxRxParam *blockParam, uint16_t txPos, uint16_t rxPos)
{
  uint16_t maxInfLen;
...
/*******************************************************************************/
ReturnCode RfalNfcClass::rfalNfcDepStartPduTransceive(rfalNfcDepPduTxRxParam param)
{
...
/*******************************************************************************/
ReturnCode RfalNfcClass::rfalNfcDepGetPduTransceiveStatus(void)
{
  ReturnCode          ret;
  rfalNfcDepTxRxParam txRxParam;
